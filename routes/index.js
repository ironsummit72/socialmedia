...
router.get('/',isloggedIn,async function(req, res, next) {
  ...
  if(userData!==null)
  {
    const {firstname,lastname,displaypicture,followers,following,coverpicture,bio}=userData
    let ownProfile=false;
    if(username===req.user.username){
      ownProfile=true
  }
  const ownerpicture=await loggedInuserDetails(req.user.username)
    res.render('profile',{username,firstname,lastname,ownprofile:ownProfile,displaypicture,ownerpicture,followers,following,coverpicture,bio})
  }else{
...
router.get('/profile/:username',isloggedIn,async function(req, res, next) {
  ...
  if(userData!==null){
    const {firstname,lastname,displaypicture,coverpicture,followers,following,bio}=userData
    let ownProfile=false;
    if(username===req.user.username){
        ownProfile=true
    }
const ownerpicture=await loggedInuserDetails(req.user.username)
    res.render('profile', { username,firstname,lastname,ownprofile:ownProfile,displaypicture,ownerpicture,coverpicture,followers,following,bio})
  }else{
    res.render('nouser',{error:"user does not exist"})
  }
